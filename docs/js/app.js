...
/* ---------- CAIXA DO MES ----------
   A Karol quer decidir sem abrir o banco: quanto tem, quanto ainda entra,
   quanto ainda falta pagar.

   CUIDADO com a armadilha: NAO da pra somar "gasto do mes" (competencia)
   com "fatura a vencer" (caixa). A fatura de setembro contem compras de
   setembro que JA estao no gasto do mes - subtrair as duas coisas conta o
   mesmo dinheiro duas vezes. Por isso este card e 100% caixa: parte do
   saldo em conta, soma o que entra e desconta o que sai. Gasto por
   competencia fica nos outros cards.

   Fica de fora do "em conta" o Itau investido e a caixinha do Nubank: e
   reserva, nao dinheiro do mes. Somar ali daria a impressao de ter 76 mil
   pra gastar.

   Este repositorio e PUBLICO: nenhum valor de salario entra aqui. O que
   ainda falta entrar sai de (renda recorrente do backend privado) menos
   (renda que ja caiu no mes). */
function renderCaixaPrevisto(){
  ...
  var corrente=0, guardado=[];
  (D.reservas||[]).forEach(function(r){
    var n=(r.nome||'').toLowerCase();
    if(n.indexOf('devolver')>=0) return;
    if(n.indexOf('conta')>=0 && n.indexOf('investido')<0) corrente+=r.valor;
    else guardado.push(r);
  });
  ...
  box.innerHTML=
   ...
   '</div>'+
   (guardado.length
     ? '<div style="font-size:11.5px;color:var(--ink-3);margin-top:10px">Fora dessa conta, guardado: '
       + guardado.map(function(r){ return esc_(r.nome)+' '+BRL(r.valor); }).join(' \u00b7 ')
       + '. Isso \u00e9 reserva \u2014 de prop\u00f3sito n\u00e3o entra no que d\u00e1 pra gastar no m\u00eas.</div>'
     : '');
